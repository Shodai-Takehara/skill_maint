    ...
    setTeamMembers([
      {
        ...
        certifications: [
          { name: 'フォークリフト運転技能講習', expiry: '2025-06-15', status: 'valid' },
          { name: '第二種電気工事士', expiry: '2025-03-20', status: 'expiring' },
          { name: 'クレーン運転士', expiry: '2026-01-10', status: 'valid' }
        ],
        workHistory: [
          { equipment: 'CNCフライス盤 #3', date: '2024-12-29', type: '定期保守' },
      ...
      },
      {
        ...
        certifications: [
          ...
          { name: '高圧ガス製造保安責任者', expiry: '2025-02-10', status: 'expiring' },
          { name: '労働安全衛生管理者', expiry: '2026-09-15', status: 'valid' }
        ],
        workHistory: [
          { equipment: '溶接機 #7', date: '2024-12-28', type: '電気系統点検' },
          { equipment: 'コンプレッサー #2', date: '2024-12-22', type: '定期保守' },
          { equipment: 'CNCフライス盤 #1', date: '2024-12-18', type: 'PLC調整' }
        ],
        joinDate: '2021-07-15'
      },
      {
        ...
        certifications: [
          ...
          { name: 'アーク溶接特別教育', expiry: '2024-11-30', status: 'expired' },
        ...
        workHistory: [
          ...
          { equipment: 'CNCフライス盤 #2', date: '2024-12-19', type: '加工作業' }
        ],
        joinDate: '2019-10-01'
      },
      {
        ...
        certifications: [
          ...
          { name: '衛生管理者第一種', expiry: '2025-01-15', status: 'expiring' }
        ],
        workHistory: [
          { equipment: 'プレス機 #7', date: '2024-12-27', type: '品質検査' },
          { equipment: 'CNCフライス盤 #3', date: '2024-12-23', type: '寸法測定' },
      ...
      },
      {
        ...
        certifications: [
          { name: '高圧ガス製造保安責任者', expiry: '2025-11-30', status: 'valid' },
          { name: 'ボイラー技士二級', expiry: '2025-05-10', status: 'valid' },
          { name: '危険物取扱者乙種4類', expiry: '2026-12-25', status: 'valid' }
        ],
        workHistory: [
          { equipment: 'コンプレッサー #3', date: '2024-12-30', type: '定期点検' },
          { equipment: '空圧システム #1', date: '2024-12-26', type: '配管修理' },
          { equipment: '油圧ポンプ #2', date: '2024-12-22', type: 'オイル交換' }
        ],
        joinDate: '2022-03-15'
      }
    ]);
    ...
    <MainLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2">チーム管理</h2>
              ...
              <Link href={`/team/${member.id}`}>
                <CardContent className="p-6">
                  <div className="flex items-start space-x-4 mb-4">
                    ...
                    <div className="text-right">
                      <Badge variant="outline" className="text-xs">
                        入社: {new Date(member.joinDate).toLocaleDateString('ja-JP', { year: 'numeric', month: 'numeric' })}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {/* スキル */}
                    <div>
                      <h4 className="text-sm font-medium text-gray-900 mb-2">保有スキル</h4>
                    ...
                    {/* 資格・認定 */}
                    <div>
                      <h4 className="text-sm font-medium text-gray-900 mb-2">資格・認定</h4>
                      <div className="space-y-1">
                        {member.certifications.slice(0, 2).map((cert, index) => (
                          <div key={index} className="flex items-center justify-between text-xs">
                            <span className="text-gray-600 truncate flex-1">{cert.name}</span>
                            <Badge className={`ml-2 ${getCertificationBadgeColor(cert.status)}`}>
                    ...
                    {/* 最近の作業履歴 */}
                    <div>
                      <h4 className="text-sm font-medium text-gray-900 mb-2">最近の作業</h4>
                      <div className="space-y-1">
                        {member.workHistory.slice(0, 2).map((work, index) => (
                          <div key={index} className="flex items-center justify-between text-xs">
                            <div className="flex-1">
                              <span className="text-gray-900">{work.equipment}</span>
                              <span className="text-gray-500 ml-1">- {work.type}</span>
                            </div>
                            <span className="text-gray-500 ml-2">
                              {new Date(work.date).toLocaleDateString('ja-JP', { month: 'numeric', day: 'numeric' })}
                            </span>
                          </div>
                        ))}
                      </div>
