    ...
    <MainLayout>
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 font-medium">製造設備の保守状況とチームパフォーマンスの概要</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="bg-white/80 backdrop-blur-sm shadow-sm border border-gray-200/50 hover:shadow-md transition-all duration-200">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">総設備数</p>
          ...
          <Card className="bg-white/80 backdrop-blur-sm shadow-sm border border-gray-200/50 hover:shadow-md transition-all duration-200">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">アクティブ作業指示</p>
          ...
          <Card className="bg-white/80 backdrop-blur-sm shadow-sm border border-gray-200/50 hover:shadow-md transition-all duration-200">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">保守完了率</p>
          ...
          <Card className="bg-white/80 backdrop-blur-sm shadow-sm border border-gray-200/50 hover:shadow-md transition-all duration-200">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-semibold text-gray-600 mb-1">チームスキルカバレッジ</p>
            ...
            <CardHeader className="border-b border-gray-100/50">
              <div className="flex items-center justify-between">
                ...
                <Link href="/work-orders">
                  <Button variant="ghost" size="sm" className="text-gray-700 hover:text-gray-900 hover:bg-gray-100/80 font-medium">
                  ...
                  <div key={order.id} className="p-4 hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <h4 className="font-semibold text-gray-900 mb-1">{order.equipment}</h4>
                        <p className="text-sm text-gray-600 mb-2 font-medium">{order.description}</p>
                        <p className="text-xs text-gray-500 font-medium">担当: {order.assignee}</p>
                      </div>
                      <div className="flex flex-col items-end space-y-2">
                        <Badge className={`${getPriorityColor(order.priority)} font-medium`}>
                ...
                {maintenanceSchedule.map((item) => (
                  <div key={item.id} className="p-4 hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex-1">
                        <h4 className="font-semibold text-gray-900 mb-1">{item.equipment}</h4>
                        <p className="text-sm text-gray-600 mb-2 font-medium">{item.type}</p>
                        <p className="text-xs text-gray-500 font-medium">担当: {item.technician}</p>
                      </div>
                      <div className="text-right">
                        ...
                        <Badge variant="outline" className="text-xs font-medium border-gray-300 text-gray-600">
                          {Math.ceil((new Date(item.dueDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))}日後
                        </Badge>
                      </div>
                    </div>
                  </div>
